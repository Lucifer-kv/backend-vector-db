...
class VectorDB:
    def __init__(self):
        ...
        self.indexes: Dict[str, HNSWIndex] = {}
    ...
    def create_library(self, library: Library):
        with self.lock:
            ...
            self.indexes[library.id] = HNSWIndex()
    ...
    def add_chunk(self, library_id: str, document_id: str, chunk: Chunk):
        with self.lock:
            ...
            self.indexes[library_id].add(chunk.id, chunk.embedding)
                ...
                if document.id == document_id:
                    for i, chunk in enumerate(document.chunks):
                        if chunk.id == chunk_id:
                            ...
                            self.indexes[library_id].add(updated_chunk.id, updated_chunk.embedding)
                            return
    ...
    def search(self, library_id: str, query_embedding: List[float], k: int) -> List[SearchResult]:
        with self.lock:
            if library_id not in self.libraries:
                raise ValueError(f"Library with ID {library_id} not found")
            return self.indexes[library_id].search(query_embedding, k)
